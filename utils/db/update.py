...
async def update_user(telegram_id, name=False, wins=False, losses=False):
    conn = await get_conn()
    if name:
        await conn.execute('UPDATE users1 SET name = $1 WHERE telegram_id = $2', name, telegram_id)
    if wins:
        await conn.execute('UPDATE users1 SET wins = wins + 1 WHERE telegram_id = $1', telegram_id)
    if losses:
        await conn.execute('UPDATE users1 SET losses = losses + 1 WHERE telegram_id = $1', telegram_id)
    await conn.close()
